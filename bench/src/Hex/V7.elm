module Hex.V7 exposing (toString, fromString)

{-| V7: toString reads 2x Int32 per iteration (8 bytes at a time),
with lookup table and pre-joined 8-char strings.
fromString uses Array-based nibble lookup instead of if-else branching.
-}
...
decodeLoop fullWords remainder =
    Decode.loop (LoopState fullWords remainder []) loopStep


loopStep : LoopState -> Decoder (Decode.Step LoopState String)
loopStep state =
    if state.words >= 2 then
        Decode.map2
            (\w1 w2 ->
                Decode.Loop
                    (LoopState (state.words - 2) state.rem (word32ToHex w2 :: word32ToHex w1 :: state.acc))
            )
        ...
        Decode.unsignedInt32 BE
            |> Decode.map
                (\word ->
                    Decode.Loop (LoopState 0 state.rem (word32ToHex word :: state.acc))
                )
        ...
        Decode.unsignedInt8
            |> Decode.map
                (\byte ->
                    Decode.Loop (LoopState 0 (state.rem - 1) (lookupByte byte :: state.acc))
                )
...
-- fromString: Array-based nibble lookup (avoid if-else branching)
...
hexPairAt hex offset =
    let
        hi =
            nibbleLookup (String.slice offset (offset + 1) hex)

        lo =
            nibbleLookup (String.slice (offset + 1) (offset + 2) hex)
    in
    if hi < 0 || lo < 0 then
        -1
        ...
        Just ( c, _ ) ->
            let
                code =
                    Char.toCode c
            in
            case Array.get code nibbleTable of
                Just v ->
                    v
...
{-| Array indexed by char code. Valid hex digits map to 0-15, all others to -1.
Covers char codes 0 through 102 ('f').
-}
nibbleTable : Array Int
nibbleTable =
    Array.initialize 103
        (\i ->
            if i >= 97 && i <= 102 then
                i - 87

            else if i >= 65 && i <= 70 then
                i - 55

            else if i >= 48 && i <= 57 then
                i - 48

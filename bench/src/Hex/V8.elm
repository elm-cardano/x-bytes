module Hex.V8 exposing (fromString, toString)

{-| V8: Best-of combo.
toString: map2 (2x Int32 per iter) + lookup table + pre-joined 8-char strings per word.
fromString: String.toList upfront + pattern-match 8 chars into Int32 encoders.
-}
...
encStep state =
    if state.words >= 2 then
        Decode.map2
            (\w1 w2 ->
                Decode.Loop
                    (EncState (state.words - 2) state.rem (word32ToHex w2 :: word32ToHex w1 :: state.acc))
            )
...
-- fromString: String.toList upfront + pattern-match 8 chars at a time

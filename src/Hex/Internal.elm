module Hex.Internal exposing (hexPairAt, isValidHex, lookupByte)

{-| Internal shared helpers for hex encoding/decoding.

@docs hexPairAt, isValidHex, lookupByte
...
{-| Check whether a hex string contains only valid hex characters.
Assumes the string has even length (caller must check).
Uses the same batched 4-pair-at-a-time walk as `Hex.toBytes`, but only
validates — no Encoder nodes, no Int32 packing, no Bytes allocation.
-}
...
isValidHex hex =
    let
        ...
        fullWords =
            byteCount // 4
    in
    case validateWords hex 0 fullWords of
        Just offset ->
            validateRemainder hex offset (modBy 4 byteCount)

        Nothing ->
            False


validateWords : String -> Int -> Int -> Maybe Int
validateWords hex offset remaining =
    if remaining > 0 then
        let
            b0 : Int
            b0 =
        ...
        if b0 < 0 || b1 < 0 || b2 < 0 || b3 < 0 then
            ...
            validateWords hex (offset + 8) (remaining - 1)

    else
        Just offset


validateRemainder : String -> Int -> Int -> Bool
validateRemainder hex offset remaining =
    if remaining > 0 then
        if hexPairAt hex offset < 0 then
            False

        else
            validateRemainder hex (offset + 2) (remaining - 1)
